...
class GuessersList:
	...
	def add_race_results_to_tenth_place(self, race_results: list[list[str]]):
		...
		for race_number, race in enumerate(race_results):
			for guesser in self.list_of_guessers:
				guesser.add_10th_place_result(race_number, race[1:])
	...
	def evaluate_antall(self):
		...
		for _, stats_key, guesser_key in Stats.get_categories_in_antall():
